...
- ✅ Frontend Angular consumiendo microservicios
...
|-----------|-----------|--------|-----------------|
| **Angular Web** | Angular 17+ | 5173 | Interfaz de usuario |
...
graph TB
    ...
    classDef database fill:#f7b731,stroke:#333,stroke-width:2px,color:#000
    classDef infra fill:#a29bfe,stroke:#333,stroke-width:2px,color:#000
    classDef aspire fill:#512da8,stroke:#333,stroke-width:3px,color:#fff

    %% FRONTEND
    Angular["🖥️ Angular SPA<br/>OrderFlow.Web<br/>:5173"]

    %% API GATEWAY
    Gateway["🚪 API Gateway<br/>YARP + Polly<br/>Rate Limiting"]

    %% MICROSERVICIOS
    subgraph Services["⚙️ MICROSERVICES"]
        Identity["🔐 Identity<br/>JWT Auth<br/>ASP.NET Identity"]
        Catalog["📦 Catalog<br/>Products<br/>Inventory"]
        Orders["🛒 Orders<br/>Order Processing<br/>Validation"]
        Customers["👥 Customers<br/>Profiles<br/>Management"]
        Notifications["📧 Notifications<br/>Email<br/>Webhooks"]
    end
    ...
    subgraph Infrastructure["🏗️ Shared Infrastructure"]
        ...
        RabbitMQ["🐰 RabbitMQ<br/>Async Events"]
    end

    %% ASPIRE
    Aspire["🚀 .NET Aspire AppHost<br/>Orchestration + Service Discovery<br/>Health Checks + Telemetry"]

    %% FLUJO PRINCIPAL
    Angular -->|HTTPS| Gateway
    Gateway -->|HTTP| Identity
    Gateway -->|HTTP + JWT| Catalog
    Gateway -->|HTTP + JWT| Orders
    Gateway -->|HTTP + JWT| Customers

    %% COMUNICACIÓN ENTRE SERVICIOS
    Orders -->|"HTTP<br/>Check Stock"| Catalog
    Orders -->|"HTTP<br/>Get Customer"| Customers
    ...
    %% EVENTOS ASÍNCRONOS
    Orders -.->|OrderCreated| RabbitMQ
    Catalog -.->|StockUpdated| RabbitMQ
    Customers -.->|CustomerRegistered| RabbitMQ
    RabbitMQ -.->|Events| Notifications

    %% ASPIRE GESTIONA TODO
    Aspire -.->|Manages| Services
    Aspire -.->|Manages| Gateway
    Aspire -.->|Manages| Angular
    Aspire -.->|Provisions| Databases
    Aspire -.->|Provisions| Infrastructure

    %% ESTILOS
    class Angular frontend
    class Gateway gateway
    class Identity,Catalog,Orders,Customers,Notifications service
    class DB1,DB2,DB3,DB4 database
    class Redis,RabbitMQ infra
    class Aspire aspire
